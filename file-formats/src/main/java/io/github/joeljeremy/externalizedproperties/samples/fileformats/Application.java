...
public class Application {
  ...
  private static ExternalizedProperties buildExternalizedProperties() throws IOException {
    return ExternalizedProperties.builder()
        .defaults()
        .resolvers(propertiesFile(), jsonFile(), yamlFile(), xmlFile(), hoconFile())
        .build();
  }

  private static ResourceResolver propertiesFile() throws IOException {
    return ResourceResolver.fromUrl(Application.class.getResource("/app.properties"));
  }

  private static ResourceResolver jsonFile() throws IOException {
    return ResourceResolver.fromUrl(Application.class.getResource("/app.json"), new JsonReader());
  }

  private static ResourceResolver yamlFile() throws IOException {
    return ResourceResolver.fromUrl(Application.class.getResource("/app.yaml"), new YamlReader());
  }

  private static ResourceResolver xmlFile() throws IOException {
    return ResourceResolver.fromUrl(Application.class.getResource("/app.xml"), new XmlReader());
  }

  private static ResourceResolver hoconFile() throws IOException {
    return ResourceResolver.fromUrl(Application.class.getResource("/app.conf"), new HoconReader());
  }
}
